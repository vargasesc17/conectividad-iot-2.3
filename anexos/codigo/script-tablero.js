...
var SITIOS = ["neusa", "cogua", "tomine"];
var VARIABLES = ["temp_ambiente", "humedad", "temp_agua", "luz", "presion"];
var UNIDADES = {
	"temp_ambiente": "°C",
	"humedad": "%",
	"temp_agua": "°C",
...
function crearTablero() {
	...
	var html = "<div class='panel panel-default' id='tablero'>";
	html += "<div class='panel-heading'>Tablero de Monitoreo Hídrico</div>";
	html += "<div class='panel-body'><table class='table'>";
	html += "<tr><th>Variable</th>";

	for (var i = 0; i < SITIOS.length; i++) {
		html += "<th>" + SITIOS[i].toUpperCase() + "</th>";
	}
	html += "</tr>";

	for (var v = 0; v < VARIABLES.length; v++) {
		html += "<tr><td>" + VARIABLES[v] + "</td>";
		for (var s = 0; s < SITIOS.length; s++) {
			html += "<td id='celda-" + SITIOS[s] + "-" + VARIABLES[v] + "'>—</td>";
		}
		html += "</tr>";
	}

	html += "</table></div></div>";
	$("#messagesPanel").before(html);
	$("#tablero").css("color", "#fff");
	$("#tablero td, #tablero th").css("color", "#fff");
}

function message_received(data) {
	crearTablero();

	var partes = data.topic.split("/");
	if (partes.length === 3) {
		var sitio = partes[1];
		var variable = partes[2];
		var unidad = UNIDADES[variable] || "";
		$("#celda-" + sitio + "-" + variable).text(data.payload + " " + unidad);
	}
